...
%     close all; clear; clc;
    ...
    if(method==2)        
        kk=[0.0:1e-7:1e-5,2e-5:5e-6:2e-4,3e-4:1e-4:6e-3]; theta=0*pi/6;
        nk=length(kk);
        ...
        for is=1:ns
            ...
            hold on; xlabel('k'); ylabel('\omega_r'); xlim([min(kk),max(kk)]);
            title(['(a) \omega_r vs k, \theta=',...
                num2str(theta*180/pi),'^{\circ}']);
            subplot(122); 
            plot(kk,imag(ww2(:,is)),'.','Color',clr,'LineWidth',2); 
            hold on; xlabel('k'); ylabel('\omega_i'); xlim([min(kk),max(kk)]);
...
function initial
    ...
    B0=8.0E-9; c2=(2.9979E8)^2; epsilon0=8.854E-12 ; mu0=1/(c2*epsilon0);
