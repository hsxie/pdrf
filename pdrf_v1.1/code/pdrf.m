...
%
% test run:
%   close all; clear; clc; [M,A,w1,w2]=pdrf;
%
% Note: 
...
function [M,A,w1,w2]=pdrf  % some test runs
...
%     close all; clear; clc;
    ...
    if(method==2)
        ...
        kmin=0.0; kmax=10.0; dk=0.1;  theta=pi/4;
        kk=kmin:dk:kmax;
        nk=length(kk);
...
function initial
    ...
    % other initial parameters, normalizations
    B0=1.0; c2=1; epsilon0=1; mu0=1/(c2*epsilon0);
    
    gammaTs=0.*qs+1.0; % adiabatic coefficients, modify here for para/perp
